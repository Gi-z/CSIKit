...
class IWLBeamformReader:
    ...
    def __init__(self, filename, scaled=False):
        ...
        if os.path.exists(filename):
            with open(filename, "rb") as file:
                data = file.read()
                self.csi_trace = self.read_bf_file(data)
                self.timestamp = scale_timestamps(self.csi_trace)
        else:
            raise Exception("File not found: {}".format(filename))

    @staticmethod
    def read_bfee(header, data, i=0, filename="", scaled=False):
        """
        ...
        if length != data_length:
            return print_length_error(length, data_length, i, filename)
    ...
    def read_bf_entry(data):
        ...
        csi_block = IWLBeamformReader.read_bfee(csi_header, all_data)

        return csi_block

    def read_bf_file(self, data):
        """
        ...
        while (length - cur) > 100:
            ...
            if code == VALID_BEAMFORMING_MEASUREMENT:
                ...
                csi_data = IWLBeamformReader.read_bfee(header_block, data_block, expected_count, self.scaled)
                if csi_data:
