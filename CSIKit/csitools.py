...
def to_json(csi_trace):
    """
        ...
        Parameters:
            csi_trace List[dict] -- A list of CSI frame structs as generated by read_bf_file. Also works with single csi entry.
    """
    def default(prop):
        if "complex" in str(type(prop)):
            return str(prop)
        if "numpy" in str(type(prop)):
            # print("Prop is numpy: {} prop -> {}".format(type(prop), prop))
            return prop.tolist()
